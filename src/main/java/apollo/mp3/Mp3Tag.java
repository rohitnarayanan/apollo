...
public class Mp3Tag extends DataBean implements Comparable<Mp3Tag> {
	 ...
	 * 
	 */
	private static final String DELETE_FIELD_CONSTANT = "|~|";
	...
	public Mp3Tag(Path aMp3Path) {
		...
		this.fileName = NIOUtils.getBaseName(aMp3Path);
		...
		for (FieldKey key : standardFields) {
			...
			if (ObjectUtils.isEmpty(fieldList) || (fieldList.size() > 1)) {
				addError("Invalid value for [" + key + "]  [" + JSONUtils.serialize(fieldList) + "]");
			}
		}
		...
		while (fieldIterator.hasNext()) {
			...
			addError("Invalid frame for [" + field.getId() + "]");
		}
	}
	...
	public Mp3Tag(Path aMp3Path, List<String> aTokens) {
		...
		for (String token : aTokens) {
			if (token.endsWith(">")) {
				...
				if (index < 0) {
					throw new ApplicationException("token not found:" + token);
				}
				...
				if (!CommonUtils.isEmpty(fieldValue) && (field == null)) {
		...
		if (fieldQueue.size() > 0) {
			throw new ApplicationException("[%s] unmapped fields remaining", fieldQueue.size());
		}
	 ...
	 */
	public void extractCommonTag(Mp3Tag aMp3Tag) throws ApplicationException {
		...
		for (String field : fieldList) {
			...
			if (!compare(getField(field), aMp3Tag.getField(field))) {
				setField(field, CommonConstants.EMPTY_STRING);
			}
		}
	...
	private String getField(String aFieldName) {
		...
		case "artwork":
			return this.artwork;
		default:
			throw new ApplicationException("unknown field [%s]", aFieldName);
		}
	}
	 ...
	 * @throws ApplicationException
	 */
	public void save(int aSaveFlag) throws ApplicationException {
	 ...
	 *                  <li>2: Write into available tag else throw an exception if
	 *                  tag not available</li>
	 *                  <li>3: Write into a new tag, ignore if tag already
	 *                  present</li>
	 *                  <li>4: Write into a new tag, throw an exception if tag
	 *                  already present</li>
	 *                  <li>5: Always write into a new tag, delete if a tag is
	 *                  present</li>
	 *                  <li>6: Do not do anything. Test mode</li>
	 *                  </ul>
	 * @param aMp3Path  Path of the mp3 file to which the tag has to be written to
	 * @throws ApplicationException
	 */
	public void save(int aSaveFlag, Path aMp3Path) throws ApplicationException {
		try {
			if (aSaveFlag == 6) {
			...
			if (tag == null) {
				...
				if (aSaveFlag == 2) {
					throw new ApplicationException("No Tag available for [%s]", this.filePath);
				}
				...
				if (aSaveFlag == 4) {
					throw new ApplicationException("Tag already present for [%s]", this.filePath);
				}
			}
